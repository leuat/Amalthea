...
namespace LemonSpawn
{
    ...
    public class C2DMap
    {
        ...
        private float getCrater(float d, float size, Vector2 p)
        {
            float dist = d - size;
            float ridge  = Mathf.Exp(-(dist * dist) * 10000f) * Mathf.Sqrt(size) * 0.5f;
            if (dist < 0)
                ridge -= Mathf.Clamp(d,0,0.1f)*2;

            float t = size;
            float a = 13.345f;
            ridge = ridge*Mathf.Clamp(Mathf.PerlinNoise(p.x*a, p.y*a),0,1);

            return ridge;
            //float valley =  Mathf.Clamp(0.020f/d*size*1, 0, 0.1f);
            //return  ridge- valley;
        }


        private float findRepeatDistance(Vector2 a, Vector3 b)
        {
            float dx = Mathf.Min( Mathf.Abs(a.x - b.x), Mathf.Abs(a.x - b.x + sizeX));
            dx = Mathf.Min(dx, Mathf.Abs(a.x - b.x - sizeX));
            float dy = Mathf.Min(Mathf.Abs(a.y - b.y), Mathf.Abs(a.y - b.y + sizeY));
            dy = Mathf.Min(dy, Mathf.Abs(a.y - b.y - sizeY));

            return Mathf.Sqrt(dx * dx + dy * dy);

        }

        public void Craters(float number, float scale, System.Random rnd)
        {
            List<Vector3> craters = new List<Vector3>();
            for(int i=0;i<number;i++)
            {
                Vector3 v = new Vector3();
                v.x = rnd.Next() % sizeX;
                v.y = rnd.Next() % sizeY;
                v.z = 0.001f + Mathf.Abs(Util.RandomGauss(0, 0.15f, rnd));
                v.z *= v.z; 
//                Debug.Log(v.z);
                craters.Add(v);
            }
            ...
            for (int i = 0; i < sizeX; i++)
            {
                for (int j = 0; j < sizeY; j++)
                {
                    p.Set(i, j);
                    float ss = 125.25f;
                    float amp = 4 / 256 * sizeX;
                    p.x += amp*Mathf.PerlinNoise(p.x/sizeX*ss, p.y/sizeY*ss);
                    ss = 193.345f;
                    p.y += amp * Mathf.PerlinNoise(p.x / sizeX*ss, p.y / sizeY*ss);

                    float h = 0.5f;
                    foreach (Vector3 c in craters)
                    {
                        Vector2 cp = new Vector2(c.x, c.y);
                        float d = findRepeatDistance(cp, p);

                        h += getCrater(d/sizeX, c.z, p/sizeX);
