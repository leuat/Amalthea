...
#include "Include/Atmosphere.cginc"
		...
		float getNoiseOctaves(float3 pos, float freq, float per, int N, float t, float add) {
			float total = 0;
			float amplitude = 1;
			float maxAmplitude = 0;
			float3 shift = float3(0.1234123, 0.2123521, 0.2591723);
			for (int i = 0; i < N; i++) {
				float v = 0.15/(noise(pos * 0.3*freq + shift*add*t*cos(i-t*0.64)*sin(i+t))+0.02);//cos(t*_Time*0.1)*sin(i*t*_Time*0.00712));
...
//			globalLight = pow(globalLight, 1);
			...
			float val = (getNoiseOctaves(pos*73.23, 1, 1, 9,1, _Time*1))*1.0 -0.0;

			float s = 0.4;
			float t1 = getNoiseOctaves(pos * 4.5, 4.23, 0.5, 4,0.83456, 0) - s;
			t1 = max(t1, 0.0);
			val = val - t1*0.3;

//			float t2 = getNoiseOctaves(pos * 2.123, 1.5, , 4,1.2354) - 0.6;
	//		t2 = max(t2, 0.0);
		//	val = val + t2*5;
